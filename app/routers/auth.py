...
@router.get("/email_verify", status_code=status.HTTP_204_NO_CONTENT)
...
@router.get("/email_change_verify", status_code=status.HTTP_204_NO_CONTENT)
def email_change_verify(token: str, db: DbSessionDep) -> None:
    ...
    try:
        user = user_service.get_user_by_id(token_data["user_id"], db)
    except NotFoundError:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND) from None
